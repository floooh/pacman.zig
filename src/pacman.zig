...
fn nearEqual(v0: ivec2, v1: ivec2, tolerance: i16) bool {
    ...
    const a: ivec2 = .{ if (d[0] < 0) -d[0] else d[0], if (d[1] < 0) -d[1] else d[1] };
...
fn clampedTilePos(tile_pos: ivec2) ivec2 {
    return .{ math.clamp(tile_pos[0], 0, DisplayTilesX - 1), math.clamp(tile_pos[1], 3, DisplayTilesY - 3) };
}
...
fn move(pixel_pos: ivec2, dir: Dir, allow_cornering: bool) ivec2 {
    ...
    if (allow_cornering) {
        ...
        if (dir_vec[0] != 0) {
            if (dist_mid[1] < 0) {
                pos[1] -= 1;
            } else if (dist_mid[1] > 0) {
                pos[1] += 1;
            }
        } else if (dir_vec[1] != 0) {
            if (dist_mid[0] < 0) {
                pos[0] -= 1;
            } else if (dist_mid[0] > 0) {
                pos[0] += 1;
            }
        }
    }
...
fn gameRoundInit() void {
    ...
    state.game.pacman = .{ .actor = .{ .dir = .Left, .pos = .{ 14 * 8, 26 * 8 + 4 } } };
...
fn gameUpdateSprites() void {
    ...
    if (state.game.active_fruit == .None) {
        ...
        spriteFruit().* = .{ .enabled = true, .pos = .{ 13 * TileWidth, 19 * TileHeight + TileHeight / 2 }, .tile = fruitSpriteCode(state.game.active_fruit), .color = fruitColorCode(state.game.active_fruit) };
    }
}
...
fn introTick() void {
    ...
    for (names) |name, i| {
        ...
        if (afterOnce(state.intro.started, delay)) {
            gfxColorTile(.{ 4, y + 0 }, color, TileCodeGhost + 0);
            gfxColorTile(.{ 5, y + 0 }, color, TileCodeGhost + 1);
            gfxColorTile(.{ 4, y + 1 }, color, TileCodeGhost + 2);
            gfxColorTile(.{ 5, y + 1 }, color, TileCodeGhost + 3);
            gfxColorTile(.{ 4, y + 2 }, color, TileCodeGhost + 4);
            gfxColorTile(.{ 5, y + 2 }, color, TileCodeGhost + 5);
        }
